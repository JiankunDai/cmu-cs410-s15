...
/* http://www.ittybittycomputers.com/IttyBitty/TinyBasic/TinyBasic.c but
 * hammered on by Josh Zimmerman so it builds on Pebbles with libopen.
 */
...
#define FileType           int
#define IoFileClose(fi)    close(fi)
#define InFileChar(fi)     CfileRead(fi)
#define OutFileChar(fi,ch) do { char __ch = ch; write(fi,&__ch,1); } while(0)
#define ScreenChar(ch)     do { char __ch = ch; print(1, &__ch); } while(0)
int num_lines_loaded = 0;
char getchar(void) {
    char buf[1];
    int res = readline(1, buf);
    assert(res == 1);
    // XXX TODO: this is bad.
    ScreenChar('\b');
    return buf[0];
}
...
FileType inFile = -1; /* from option '-i' or user menu/button */
FileType oFile = -1;  /* from option '-o' or user menu/button */
...
void Ouch(char ch) {                         /* output char to stdout */
  if (inFile >= 0 && ch == ':') {
      ...
      // This is a disgusting hack, but we really don't want the ':'s printed
      // for every line that's loaded.
      return;
  }
  if (oFile >= 0) {                   /* there is an output file.. */
...
char Inch(void) {          /* read input character from stdin or file */
  char ch;
  if (inFile >= 0) {        /* there is a file to get input from.. */
    ch = InFileChar(inFile);
    if (ch == '\n') ch = '\r';
    if (ch == '\0') {          /* switch over to console input at eof */
      IoFileClose(inFile);
      inFile = -1;
      printf("Read file. Type 'run' to execute!\n");
    }
    else {
  ...
  if (oFile >= 0) OutFileChar(oFile,ch); /* echo it to output file */
  if (ch == '\n') {
...
/* sequence is [not] output.                                          */
      case 39:
        InLend = InLine;
        if (inFile < 0) {
            InLend = readline(ExpnTop - InLine, (char *) &Core[InLine]);
            assert(InLend >= 0);
            InLend += InLine;
        }
        else {
        ...
        Core[InLend] = 0;
        if(strncmp((char *) &Core[InLine], "quit", 4) == 0) {
            printf("Bye!\n");
            exit(0);
        }
        BP = InLine;
...
void StartTinyBasic(char* ILtext) {
  ...
  ColdStart();
  Interp();                                               /* go do it */
  if (oFile >= 0) IoFileClose(oFile);         /* close output file */
  if (inFile >= 0) IoFileClose(inFile);        /* close input file */
  oFile = -1;
  inFile = -1;} /* ~StartTinyBasic */
...
int main(int argc, char* argv[]) {
  ...
  for (nx=1; nx<argc; nx++) {         /* look for command-line args.. */
    if (strcmp(argv[nx],"-b")==0 && ++nx<argc) {     /* alt IL file.. */
      tmpFile = open(argv[nx],O_RDONLY);
      if (tmpFile > 0) {
              len = sizefile(argv[nx]);                      /* get file size.. */
              assert(len >= 0);
              if (lseek(tmpFile,0,SEEK_SET)==0) if (len>9) {
                  len = len/8+len;            /* allow for line end expansion */
                  IL = (char*)malloc(len+1);
                  if (IL != NULL) len = read(tmpFile, IL, len);
                  IL[len] = '\0';
                  IoFileClose(tmpFile);
              }
      }
      ...
      oFile = open(argv[nx],O_RDWR);
    else if (strcmp(argv[nx],"-i")==0 && ++nx<argc)     /* input file */
      inFile = open(argv[nx],O_RDONLY);
    else if (inFile<0)  /* default (unadorned) is also input file */
      inFile = open(argv[nx],O_RDONLY);}             /* ignore other args */

#ifdef DefaultInputFile
  if (inFile<0) inFile = open(DefaultInputFile,O_RDONLY);
#endif
#ifdef DefaultOutputFile
  if (oFile<0) oFile = open(DefaultOutputFile,O_RDWR);
#endif
